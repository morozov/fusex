...
static const struct menu_item_entries menu_item_lookup[] = {
  ...
  { UI_MENU_ITEM_MACHINE_DEBUG_LOG, "/Machine/Debug Log..." },
...
ui_tape_write( void )
{
  ...
  filename = ui_get_save_filename( "FuseX - Write Tape" );
...
ui_mdr_write( int which, int saveas )
{
  ...
  snprintf( title, 80, "FuseX - Write Microdrive Cartridge %i", which + 1 );
...
#ifdef UI_WIN32
#include "ui/win32/debuglog.h"
#endif

void
ui_spectranext_debug_log_byte( int c )
{
#ifdef UI_WIN32
  win32_debuglog_append_byte( c );
#else
  (void)c;
#endif
}
